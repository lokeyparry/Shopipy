...
const List = ({token}) => {
  ...
  return (
    <div className='px-2 sm:px-8 sm:mt-14'>
      <div className="text-3xl text-center bold-30 text-gray-700 shadow-lg pb-8">Product list</div>
      <div className="flex flex-col gap-2">
        ...
        {list.map((item)=>(
          <div className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr] md:grid-cols-[1fr_3.5fr_1.5fr_1fr_1fr] items-center gap-2 p-1 bg-white rounded-xl" key={item._id}>
            ...
            <div onClick={()=>removeProduct(item._id)} className=""><TbTrash className='text-right md:text-center cursor-pointer text-lg text-red-500' /></div>
          </div>
        ))}
      </div>
    </div>
